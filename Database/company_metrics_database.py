...
class CompanyMetricsDatabase(object):
    max_num_reties = 1;
    ...
    def __init__(self, db_type, username, password, server, database, metric):
        ...
        self.metric = metric
        self.table = self.get_company_metrics_table_object(
                metric=metric,
                class_map=company_metrics.CompanyMetrics)
    ...
    def get_company_metrics_table_object(self, metric, class_map):
        metric_name = CompanyMetricsDatabase.create_metric_name(metric)
        metadata = sqlalchemy.MetaData(bind=self.engine)
        company_metrics_table = sqlalchemy.Table(
        ...
        sqlalchemy.orm.mapper(class_map, company_metrics_table)
        return company_metrics_table
    ...
    def get_metrics(self, metric_name=None):
        try:
            s = self.session()
            query = s.query(company_metrics.CompanyMetrics)
            if metric_name:
                query = query.filter_by(metric_name=metric_name)
            query = query.order_by(company_metrics.CompanyMetrics.start_date)
            data = query.all()
            s.expunge_all()
            return data
        except Exception as e:
